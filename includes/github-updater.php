...
class GitHubPluginUpdater {
  ...
  private static function get_token() {
    if (defined('GITHUB_UPDATER_TOKEN') && GITHUB_UPDATER_TOKEN) return GITHUB_UPDATER_TOKEN;
    return null;
  }

  public static function inject_github_auth($parsed_args, $url) {
    $host = parse_url($url, PHP_URL_HOST);
    if (!in_array($host, ['api.github.com', 'codeload.github.com', 'github.com'], true)) return $parsed_args;
    $token = self::get_token();
    if (!$token) return $parsed_args;
    if (!isset($parsed_args['headers'])) $parsed_args['headers'] = [];
  ...
  public function clear_latest_release_cache() {
    if ($this->cache_allowed) delete_transient($this->latest_release_cache_key);
  ...
  public function display_cache_cleared_message() {
    if (!isset($_GET['cache_cleared_' . $this->plugin_slug])) return;
  ...
  function get_plugin_info($res, $action, $args) {
    ...
    if (!$this->get_latest_release() || !$this->latest_release) return $res;
    ...
    return (object) [
      ...
      'sections' => ['Description' => $this->plugin_data['Description'], 'Updates' => $this->latest_release['body']],
      'download_link' => $this->get_download_url($this->latest_release['tag_name'])
    ];
  }

  private function get_download_url($tag_name) {
    return 'https://codeload.github.com/' . $this->plugin_data['AuthorName'] . '/' . $this->plugin_slug . '/zip/refs/tags/' . $tag_name;
  }

  private function get_latest_release() {
    if ($this->latest_release) return true;
    $transient = $this->cache_allowed ? get_transient($this->latest_release_cache_key) : null;
    if ($transient) { $this->latest_release = $transient; return true; }

    $headers = ['User-Agent' => 'WordPress/' . get_bloginfo('version') . '; ' . home_url()];
    $token = self::get_token();
    if ($token) $headers['Authorization'] = 'Bearer ' . $token;

    $response = wp_remote_get('https://api.github.com/repos/' . $this->plugin_data['AuthorName'] . '/' . $this->plugin_slug . '/releases/latest', ['headers' => $headers]);
    ...
    if (empty($this->latest_release['tag_name'])) return false;
    $this->latest_release["version"] = preg_replace('/[^0-9.]/', '', $this->latest_release["tag_name"]);

    if ($this->cache_allowed) set_transient($this->latest_release_cache_key, $this->latest_release, 5 * MINUTE_IN_SECONDS);
    return true;
  }

  public function update($transient) {
    if (empty($transient->checked)) return $transient;
    if (!$this->get_latest_release() || !$this->latest_release) return $transient;
  ...
  public function fix_source_dir($source, $remote_source, $upgrader, $hook_extra) {
    if (!isset($hook_extra['plugin']) || $hook_extra['plugin'] !== $this->plugin_file) return $source;
    $expected = trailingslashit($remote_source) . trailingslashit($this->plugin_slug);
    if ($source === $expected) return $source;
    global $wp_filesystem;
    if ($wp_filesystem->move($source, $expected)) return $expected;
