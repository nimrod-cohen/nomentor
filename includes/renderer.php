...
function nomentor_render_element($element) {
  ...
  switch ($type) {
    ...
    case 'chart': $result = nomentor_render_chart($element); break;
    default: return '';
  }
...
/**
 * Build the self-contained animated growth-chart SVG. The CSS lives INSIDE
 * the SVG (so it animates identically in the editor canvas and on the
 * published page — no global keyframes needed, no external script).
 */
function nomentor_chart_svg($props) {
  $line   = esc_attr($props['lineColor'] ?? '#111a45');
  $bar    = esc_attr($props['barColor']  ?? '#9fb4e6');
  $area   = esc_attr($props['areaColor'] ?? $line);
  $tipBg  = esc_attr($props['tipColor']  ?? $line);
  // Per-instance class prefix to keep CSS isolated when multiple charts coexist.
  $u = 'c' . wp_generate_uuid4();
  $u = str_replace('-', '', substr($u, 0, 12));
  return '<svg viewBox="0 0 560 250" role="img" aria-label="Growth chart" style="display:block;width:100%;height:auto;overflow:visible">'
...
function nomentor_render_chart($element) {
  $props = $element['props'] ?? [];
  $id    = $element['id'] ?? '';
  $outer = nomentor_build_style($props, $id);
  $mw    = $props['maxWidth'] ?? '560px';
  if ($mw !== '') $outer .= ($outer ? '; ' : '') . 'max-width: ' . esc_attr($mw) . '; margin-inline: auto';
  $outer_attr = $outer ? " style=\"{$outer}\"" : '';
  return "<div{$outer_attr}>\n" . nomentor_chart_svg($props) . "\n</div>\n";
}
